...
KinectPathSegmentation::KinectPathSegmentation(){
...
list<pair<int,int>> KinectPathSegmentation::safePathList(cv::Mat img,cv::Mat distMat){
    ...
    cv::Mat inter = cv::Mat::ones(bcheck,larg,CV_8UC1);
...
pair<int,int> KinectPathSegmentation::getCentralPath(list<pair<int,int>> paths){
    if(paths.size() < 1){
...
cv::Mat KinectPathSegmentation::markSafePaths(cv::Mat img,list<pair<int,int>> paths){
    int slice = IMGHEIGHT / 4;
    cv::Mat chosenPath = img.clone();
    for(auto path : paths){
